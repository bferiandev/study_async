...
def novo_flashcard(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'novo_flashcard.html', {'categorias': categorias, 'dificuldades': dificuldade, 'flashcards': flashcards})
...
def deletar_flashcard(request, id):
    #TODO: Fazer validação do user por segurança do sistema
...
def iniciar_desafio(request):
    if request.method == 'GET':
        categorias = Categoria.objects.all()  
        dificuldade = Flashcard.DIFICULDADE_CHOICES
        return render(request, 'iniciar_desafio.html', {'categorias': categorias, 'dificuldades': dificuldade})
    elif request.method == 'POST':
        ...
        if flashcards.count() < int(qtd_perguntas):
            messages.add_message(request, constants.ERROR, 'Diminua a Quantidade de Questões, número de flascards insuficientes para gerar desafio!')
...
def relatorio(request, id):
    ...
    return render(request, 'relatorio.html', {'desafio': desafio, 'dados_pie':dados_pie, 'categorias': name_categoria, 'dados_radar': dados_radar})